...
export default function CheckoutPage() {
  ...
  // TODO 5: guard — if there's no token in localStorage, router.push("/login").
  useEffect(() => {
  }, [router]);

  // TODO 6: placeOrder() — POST /orders with
  // { items: items.map(({ id, quantity }) => ({ productId: id, quantity })) } via authedFetch.
  // ok -> clear() + router.push(`/orders?placed=${order.id}`);
  // error -> setMessage(NICE_ERRORS[error] || error || "Something went wrong.").
