 ...
 */
/**
 * The table name is configurable, and that is not gold-plating.
 *
 * Two projects sharing one database - which is exactly what this training repo
 * does - would otherwise share one `schema_migrations` table. Their migration
 * ids collide (both start at `001`), so the second project's migrations look
 * "already applied", or the checksum guard fires on a file it has never seen.
 *
 * Each independently-migrated component needs its own ledger.
 */
const DEFAULT_MIGRATIONS_TABLE = 'schema_migrations';

/**
 * Build the ledger DDL.
 *
 * The table name is interpolated, not parameterised - identifiers cannot be
 * parameterised - so it is validated against a strict identifier pattern first.
 * That check is the whole injection defence for this function.
 *
 * Returns one statement per element, because a schema-qualified ledger needs
 * its schema to exist first, and that cannot be a migration: the migrator has
 * to write to the ledger in order to record that migration.
 */
function migrationsTableSql(table: string): string[] {
  if (!/^[a-z_][a-z0-9_]*(\.[a-z_][a-z0-9_]*)?$/i.test(table)) {
    throw new Error(`Invalid migrations table name: ${table}`);
  }

  const statements: string[] = [];
  const [schema] = table.includes('.') ? table.split('.') : [];
  if (schema) statements.push(`CREATE SCHEMA IF NOT EXISTS ${schema}`);
...
/** Apply the ledger DDL statements in order. */
async function ensureMigrationsTable(
  run: (sql: string) => Promise<unknown>,
  table: string,
): Promise<void> {
  for (const statement of migrationsTableSql(table)) await run(statement);
}
...
export interface MigrateOptions {
  ...
  /** Where the ledger lives. Give each component its own. */
...
export async function appliedMigrations(
  ...
  await ensureMigrationsTable((sql) => pool.query(sql), migrationsTable);
  const { rows } = await pool.query<AppliedMigration>(
    `SELECT id, name, checksum, applied_at FROM ${migrationsTable} ORDER BY id`,
  );
  return rows;
}
...
export async function migrateUp(
  ...
  const { logger = console, migrationsTable = DEFAULT_MIGRATIONS_TABLE } = options;
  ...
  try {
    ...
    await ensureMigrationsTable((sql) => client.query(sql), migrationsTable);

    const { rows: applied } = await client.query<AppliedMigration>(
      `SELECT id, name, checksum FROM ${migrationsTable}`,
    );
    ...
    for (const migration of migrations) {
      ...
      try {
        ...
        await client.query(
          `INSERT INTO ${migrationsTable} (id, name, checksum) VALUES ($1, $2, $3)`,
...
export async function migrateDown(
  ...
  try {
    ...
    try {
      ...
      await client.query(`DELETE FROM ${migrationsTable} WHERE id = $1`, [migration.id]);
      await client.query('COMMIT');
      return migration.id;
    } catch (error) {
      await client.query('ROLLBACK');
